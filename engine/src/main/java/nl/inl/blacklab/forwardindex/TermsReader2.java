...
public class TermsReader2 extends Terms {
     ...
     * - termDataGroups
     * 
     * TODO collapse groups of size one between the collationkey and the string hash.
     */
    private void fillTermDataGroups(TIntObjectHashMap<IntArrayList> stringHash2TermIds, THashMap<byte[], IntArrayList> collationKeyBytes2TermIds, String[] terms) {
        ...
        // in those cases where both the string hash and collation key have only 1 entry
        // (which is probably be the majority of cases)
        // we can just return the same entry in the termDataGroups array
        // which can be shrunk to fit 
        
        // but..how do we detect this?
        // because there is nothing linking them together anymore
        // so we'd need the original entry instead of the collation key
        
        // right... we have the term id, the terms[] array is still intact
        ...
        while (it2.hasNext()) {
            ...
            if (termIds.size() == 1) { // see if we can collapse with the hash entry for this term id
        ...
        // now for my final trick: reclaim the size
        final int sizeBefore = termDataGroups.length;
        final int sizeAfter = offset;
        System.out.println("Saving " + (sizeBefore - sizeAfter)/1024/1024*4 + "MB by collapsing " + collapsedGroups + " dataGroups - cool!");
    ...
    private int getTermId(String term, MatchSensitivity sensitivity) {
        ...
        if (groupIndex == -1) { return -1; } // unknown term
        return termDataGroups[groupIndex + 1]; // no need to match, collationkey does not suffer from hash collisions (or at least it shouldn't) so it's guaranteed the same term.
    }
