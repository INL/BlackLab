...
public class LogUtil {
	 ...
	 * log4j2.xml (e.g. for simple tools that don't need any fancy logging).
	 *
	...
	public static void setupBasicLoggingConfig(Level level) {
        ...
        if (config.getAppenders().size() == 0) {
	        ...
	        Appender appender = ConsoleAppender.createAppender(layout, null, Target.SYSTEM_OUT, "Console", true, false, true);
	        appender.start();
