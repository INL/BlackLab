 ...
 *
 * This is useful when it's relatively expensive to create an object, and we often
...
public abstract class SimpleResourcePool<T> {
	/**
	 * The maximum number of unused resource objects in the free pool at any one time. Once the
	 * pool is full, additional resource objects are dropped to be garbage-collected instead of
	 ...
	 *
	 * @param maxFreePoolSize
	 *            size of the resource pool. If 0, disables pooling.
	 */
	...
	/**
	 * Sets whether or not we want to do any pooling at all.
	 * If not, always creates a new resource object and never returns
	 * it to the free pool.
	 *
	...
	/**
	 * Retrieves a resource object from the free pool, or creates a new one if none are available
	 *
