...
public class SpansConstrained extends BLSpans {
	...
	/**
	 * Ensure that the current document has at least one valid match;
	 * if not, advance until we find a document with at least one valid match.
	 *
	...
	/**
	 * Ensure that the current hit is valid with respect to our constraint;
	 * if not, advance to the next valid hit.
	 *
	 * @return current hit start position if valid, NO_MORE_POSITIONS if we're done in this document
	 * @throws IOException
	 */
