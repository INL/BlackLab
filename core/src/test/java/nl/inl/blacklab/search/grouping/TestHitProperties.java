...
public class TestHitProperties {
    ...
    public void testHitPropContextWords() throws ParseException {
        ...
        HitGroup group;
        group = g.getGroup(
                new HitPropValueContextWords(hits, "word", new int[] { NO_TERM, term("The"), NO_TERM }, true));
        Assert.assertEquals(1, group.size());
        group = g.getGroup(
                new HitPropValueContextWords(hits, "word", new int[] { term("over"), term("the"), NO_TERM }, true));
        Assert.assertEquals(1, group.size());
        group = g.getGroup(
                new HitPropValueContextWords(hits, "word", new int[] { term("May"), term("the"), NO_TERM }, true));
        Assert.assertEquals(1, group.size());
        group = g.getGroup(
                new HitPropValueContextWords(hits, "word", new int[] { term("is"), term("the"), NO_TERM }, true));
        Assert.assertEquals(1, group.size());
    }
    ...
    public void testHitPropContextWordsReverse() throws ParseException {
        ...
        HitGroup group;
        group = g.getGroup(new HitPropValueContextWords(hits, "word",
