 ...
 *
 * Adapted from Lucene 5.3.0. Disabled one assert because it interferes with testing.
 *
 */
public class MyTermSpans extends Spans {
	...
	public MyTermSpans(PostingsEnum postings, Term term) {
	...
	public String toString() {
		return "spans(" + term.toString() + ")@" + (doc == -1 ? "START"
				: (doc == NO_MORE_DOCS) ? "ENDDOC" : doc + " - " + (position == NO_MORE_POSITIONS ? "ENDPOS" : position));
	}
