...
public class TestSearchHitGroups {
    ...
    private void testGroup(boolean fastPath) throws InvalidQuery {
        ...
        TextPattern tp = new TextPatternAnnotation("word", new TextPatternRegex("o.*"));
        ...
        Assert.assertEquals(title + " # groups", 2, groups.size());
        Iterator<HitGroup> it = groups.iterator();
        HitGroup g = it.next();
        Assert.assertEquals(title + " 1st group size", 1, g.size());
        Assert.assertEquals(title + " 1st group id", "or", g.identity().toString());
        g = it.next();
        Assert.assertEquals(title + " 2nd group size", 1, g.size());
        Assert.assertEquals(title + " 2nd group id", "over", g.identity().toString());
    }
