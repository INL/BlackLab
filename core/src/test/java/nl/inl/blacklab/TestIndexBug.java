...
public class TestIndexBug {
    ...
    public TestIndexBug() {
        ...
        DocumentFormats.registerFormatsInDirectories(Arrays.asList(new File("src/test/resources/bug/")));
        try {
            Indexer indexer = Indexer.createNewIndex(indexDir, "ann");
            indexer.setListener(new IndexListenerThrowOnError()); // throw on error
            try {
                // Index each of our test "documents".
                indexer.index(new File("src/test/resources/bug/0.xml"));
            } finally {
