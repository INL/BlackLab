...
NOTE: this describes the new way of indexing, using index format configuration files, available starting from BlackLab 1.7.0. There's also an advanced way of adding an input format that offers around 30% better performance, by implementing a Java class. For large corpora you reindex frequently, that might be worth it. See [here](indexing-with-blacklab.html). In the future, we hope to speed up the configuration approach to indexing, obviating the need for implementing your own Java class.
