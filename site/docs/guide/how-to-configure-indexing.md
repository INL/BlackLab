...
- `split(separator, keep)`: split the field's value on the given separator and keep only the part indicated by `keep` (0-based). If `keep` is omitted, keep the first part. If `separator` is omitted, use `;`.
