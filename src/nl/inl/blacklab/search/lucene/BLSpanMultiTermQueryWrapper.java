 ...
 * 	SpanQuery spanWildcard = new BlackLabSpanMultiTermQueryWrapper&lt;WildcardQuery&gt;(wildcard);
...
public class BLSpanMultiTermQueryWrapper<Q extends MultiTermQuery> extends SpanQuery {
	...
	/**
	 * Create a new BlackLabSpanMultiTermQueryWrapper.
	 *
	 * @param query
	 *            Query to wrap.
	 *            <p>
	 *            NOTE: This will call
	 *            {@link MultiTermQuery#setRewriteMethod(MultiTermQuery.RewriteMethod)} on the
	 *            wrapped <code>query</code>, changing its rewrite method to a suitable one for
	 *            spans. Be sure to not change the rewrite method on the wrapped query afterwards!
	 *            Doing so will throw {@link UnsupportedOperationException} on rewriting this query!
	 *            In Lucene 3.x, MultiTermQuery allows queries to rewrite to different field names,
	 *            but SpanQuery needs a fixed field. The wrapped query must therefore support
	 *            getField() or getTerm().
			...
			try {
				...
				try {
					...
					throw new IllegalArgumentException(
							"BlackLabSpanMultiTermQueryWrapper can only wrap MultiTermQueries"
	...
	public final SpanRewriteMethod getRewriteMethod() {
		...
		if (!(m instanceof SpanRewriteMethod))
			throw new UnsupportedOperationException(
					"You can only use BlackLabSpanMultiTermQueryWrapper with a suitable SpanRewriteMethod.");
	...
	public String getField() {
		try {
			...
			// BL-specific: return only base name of complex field!
			return ComplexFieldUtil.getBaseName(((Term) getTermMethod.invoke(query)).field());
		} catch (Exception e) {
	...
	public String toString(String field) {
		...
		builder.append("BlackLabSpanMultiTermQueryWrapper(");
	...
	public static abstract class SpanRewriteMethod extends MultiTermQuery.RewriteMethod {
		@Override
		public abstract SpanQuery rewrite(IndexReader reader, MultiTermQuery query)
				throws IOException;
	}
